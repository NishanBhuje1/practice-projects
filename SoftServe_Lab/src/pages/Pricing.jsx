...
function Pill({ children }) {
  return (
    <span className="inline-flex items-center rounded-full px-3 py-1 text-xs tracking-wide text-white/70 bg-white/5 ring-1 ring-white/10">
      {children}
    </span>
  );
}
  ...
  return (
    <div
      className={[
        ...
        highlight ? "ring-white/20 bg-white/[0.07]" : "",
      ].join(" ")}
    >
      {highlight ? (
      ...
      <h3 className="text-xl font-medium tracking-tight text-white/90">{title}</h3>
      <p className="mt-2 text-sm text-white/55 leading-6">{tagline}</p>

      <div className="mt-6 flex items-end gap-2">
        <div className="text-4xl font-light text-white/90">{price}</div>
        <div className="pb-1 text-sm text-white/50">AUD</div>
      </div>
      ...
      <ul className="mt-6 space-y-3">
        {features.map((f) => (
          <li key={f} className="flex gap-3 text-sm text-white/70 leading-6">
    ...
    <div className="bg-softserve relative overflow-hidden guide-lines">
      ...
      <PageSection id="pricing-hero" header={null} divider="wave" contentClassName="py-8 md:py-10">
        <div className="text-center pt-4 md:pt-6">
          ...
          <h2 className="display text-[48px] sm:text-[64px] md:text-[78px] text-white/85 mt-5">
            Simple plans that
            <br />
            scale with your business
          </h2>

          <p className="mt-6 max-w-2xl mx-auto text-sm sm:text-base leading-7 text-white/60">
      ...
      <PageSection id="setup" header="Section: Setup" divider="line" className="pt-0">
        <div className="grid gap-8 md:grid-cols-12 items-start">
          <div className="md:col-span-5">
            <h2 className="display text-5xl md:text-6xl text-white/75">
              One-Time
              <br />
              Setup
            </h2>
            <p className="mt-6 text-sm leading-7 text-white/60 max-w-sm">
              Covers onboarding, configuration, and initial implementation. Charged once per client.
            </p>
          </div>

          <div className="md:col-span-7">
            <div className="rounded-3xl bg-white/5 ring-1 ring-white/10 backdrop-blur p-8 md:p-9">
              <div className="flex items-center justify-between gap-4 flex-wrap">
                <div>
                  <p className="text-lg font-medium text-white/90">Standard Setup</p>
      ...
      <PageSection id="plans" header="Section: Plans" divider="line" className="pt-0">
        <h2 className="display text-5xl md:text-6xl text-white/75">Monthly Subscription</h2>
        <p className="mt-6 text-sm leading-7 text-white/60 max-w-2xl">
      ...
      <PageSection id="support-seo" header="Section: Add-ons" divider="none" className="pt-0">
        <div className="grid gap-6 md:grid-cols-2">
          <InfoBlock title="Support & Maintenance (Included)">
            <ul className="mt-4 space-y-2">
              {[
                ...
                <li key={x} className="flex gap-3">
                  <span className="mt-[7px] h-2 w-2 rounded-full bg-white/40" />
                  <span>{x}</span>
                </li>
              ))}
            </ul>
            ...
            <div className="mt-6 rounded-2xl bg-black/20 ring-1 ring-white/10 p-5">
              <p className="text-white/80 font-medium">Out of Scope</p>
              <p className="mt-2 text-white/60">
                <span className="text-white/80">AUD $110 / hour</span> for custom features, major redesigns,
          ...
          <InfoBlock title="SEO Services (Optional Add-on)">
            <div className="rounded-2xl bg-black/20 ring-1 ring-white/10 p-5">
              <p className="text-white/80 font-medium">Local SEO Setup</p>
              <p className="mt-1 text-white/60">AUD $400 (one-time)</p>
            ...
            <div className="mt-5 rounded-2xl bg-black/20 ring-1 ring-white/10 p-5">
              <p className="text-white/80 font-medium">Ongoing SEO</p>
              <p className="mt-1 text-white/60">AUD $300 / month</p>
              <ul className="mt-3 space-y-2">
                {[
        ...
        {/* CTA */}
        <div className="mt-10 rounded-3xl bg-white/5 ring-1 ring-white/10 backdrop-blur p-8 md:p-10 text-center">
          <p className="text-sm tracking-[0.18em] uppercase text-white/55">Ready to launch?</p>
          <h3 className="mt-4 text-3xl sm:text-4xl font-light text-white/85">
            Let’s set you up and ship fast.
          </h3>
          <p className="mt-4 text-sm sm:text-base text-white/60 max-w-2xl mx-auto leading-7">
